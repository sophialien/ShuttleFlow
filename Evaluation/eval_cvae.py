...
from GAN_models.VAE import VAE
from args_gan import get_parser

def count_parameters(model):
    return sum(p.numel() for p in model.parameters() if p.requires_grad)

def metric1(model, sample_points, test_data_path, player_list, mean, std):
    batch_size = 64
...
def main(dataset, logname):
    ...
    tr_dataset, te_dataset, _ = get_datasets(args)
    player_list = tr_dataset.get_player_list()
    mean, std = tr_dataset.get_pc_stats()

    model = VAE(args, tr_dataset.get_num_players())
    model = model.cuda()
    ckpt = torch.load(path)
    model.load_state_dict(ckpt['VAE'])
    # print("Number of trainable parameters of model: {}".format(count_parameters(model)))
    # print("Number of trainable parameters of decoder: {}".format(count_parameters(model.decoder)))
    # print("Number of trainable parameters of context_encoder: {}".format(count_parameters(model.context_encoder)))
    ...
    # for n in [10, 32, 64, 96, 128, 192, 256]:
        rmse, mae, rmse_s, mae_s = metric1(model, n, test_data_path, player_list, mean, std)
        RMSE.append(rmse)
        MAE.append(mae)
