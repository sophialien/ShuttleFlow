...
def build_model(args, input_dim, hidden_dims, context_dim, player_dim, num_blocks, conditional):
    def build_cnf():
        diffeq = ODEnet(
...
def get_hit_point_cnf(args, input_dim, zdim, pdim):
    dims = tuple(map(int, args.dims.split("-")))
    model = build_model(args, input_dim, dims, zdim, pdim, args.num_blocks, True).cuda()
    print("Number of trainable parameters of Point CNF: {}".format(count_parameters(model)))
    return model

def get_latent_cnf(args, zdim, pdim):
    dims = tuple(map(int, args.latent_dims.split("-")))
    model = build_model(args, args.z1_dim, dims, zdim, pdim, args.latent_num_blocks, False).cuda()
